...
const helper = {
  ...
  getManuallyIgnore: async (body, usersToIgnore, testMode) => {
    ...
    const keyOrId =
      type === 'issuelink' ? await helper.getAvailableIssueId(body) : utils.getIssueKey(body) || utils.getIssueId(body);
  ...
  itemsToString: items => items.reduce((acc, { field, toString }) => (field ? { ...acc, [field]: toString } : acc), {}),
